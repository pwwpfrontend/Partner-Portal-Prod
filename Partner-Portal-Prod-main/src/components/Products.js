...
const generatePDFDocument = (product) => {
  try {
    ...
    if (product.description) {
      ...
      const splitDescription = doc.splitTextToSize(normalizeDescription(product.description), 170);
          ...
          if (product.models && product.models.length > 0) {
            product.models.forEach((model, index) => {
              ...
              processedProducts.push({
                ...
                product_datasheet: product.product_datasheet || ''
              });
            });
          } else {
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        {downloadLoading === selectedProduct.id ? (
                          <>
                            <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-white"></div>
                            {selectedProduct.product_datasheet ? 'Downloading...' : 'Generating...'}
                          </>
                        ) : (
                          <>
                            <Download className="w-3 h-3" />
                            {selectedProduct.product_datasheet ? 'Download Official Datasheet' : 'Generate Datasheet'}
                          </>
                        )}
                      </button>
                    </div>
                            ...
                            <div className="flex items-center">
                              ...
                              <div className="ml-3">
                                ...
                                <div className="text-xs text-[#818181] leading-relaxed">
                                  <div className="whitespace-pre-wrap">{normalizeDescription(product.description).length > 50 
                                    ? `${normalizeDescription(product.description).substring(0, 50)}...`
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-gray-600 leading-relaxed mb-3">
                              <div className="whitespace-pre-wrap">{normalizeDescription(product.description).length > 80 
                                ? `${normalizeDescription(product.description).substring(0, 80)}...`
                                : normalizeDescription(product.description)
                              }</div>
                            </div>
                          </div>
                        ...
                        <div className="flex gap-2 mt-3 pt-3 border-t border-gray-200">
                          ...
                          >
                            ...
                            <span className="hidden sm:inline">{product.product_datasheet ? 'Official PDF' : 'Generate PDF'}</span>
                            <span className="sm:hidden">{product.product_datasheet ? 'Official' : 'Generate'}</span>
                          </button>
                        </div>
                      </div>
                    </div>
